		...
		if($file){
			...
			if($info){
	            ...
	            $saleType = input('?post.sale')?input('post.sale'):"";
		   		...
		   		/*上传到数据库*/
		   		$res = db('t_goods')->data(['goodsid'=>"default",'quantity'=>$quantity,'soldqty'=>'default','price'=>$price,'goodstatus'=>$goodstatus,'saleType'=>$saleType,'cpubtime'=>date("Y-m-d H:i:s",time()),'cshelftime'=>$cshelftime,'salelimit'=>$salelimit,'saleprice'=>$saleprice,'salePeriod'=>$salePeriod,'name'=>$name,'locateid'=>$locateid,'image'=>$image,'intro'=>$intro])->insert();
		   		/*页面显示*/
		   		if($res){
